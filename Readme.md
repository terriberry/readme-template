...
By default, the ```ASPNETCORE_ENVIRONEMNT``` is set to ```Local``` in the launchsettings.json file. This is if the project is running on your local machine, if hosted on a server set this environment to ```Development```.

### appsettings.json and environment variable
The ```appsettings.json``` file is a template auto-generated by Intent. </br>
Copy the code inside the appsettings.json and paste into ```appsettings.Local.json``` if hosted locally and ```appsettings.Development.json``` if hosted on a server.</br>
