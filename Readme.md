...
By default, the ```ASPNETCORE_ENVIRONEMNT``` is set to ```Local``` in the launchsettings.json file. This is if the project is running on your local machine, if hosted on a server set this environment to ```Development``` or ```Production```.

### appsettings.json and environment variable
The ```appsettings.json``` file is a template auto-generated by Intent. </br>
Copy the code inside the appsettings.json and paste into ```appsettings.Local.json``` if hosted locally and ```appsettings.Development.json``` or ```appsettings.Production.json``` if hosted on a server.</br>

It is **best practice** to store secret keys such as ```AWS_ACCESSKEY``` inside the environment variable (in ```launchsettings.json```) while general settings such as ```IpRateLimitOptions``` are stored inside the ```appsettings.json```.
***
